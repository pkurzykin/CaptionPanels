...
- Branding timing: исправлено создание плашек из parsed-геотегов — geotag теперь якорится к актуальным subtitle-слоям без fallback на устаревший импортный `time`, а reset для первой следующей `head_topic` рассчитывается по `geotag.out` (устраняет пропуски при переходе через границы SYNCH).
- Branding anchors: добавлена детерминированная связь `geotag -> parent subtitle` через `anchorSegId` (primary) и `anchorType+anchorBatch+anchorIndex` (fallback); для subtitle-слоёв при JSON-import теперь сохраняется `CP_SRCSEGID`, что позволяет корректно находить родительский блок после Auto Timing и смещений тайминга.
- Branding stacking/bg refresh: выровнен порядок слоёв в цепочке создания плашек (`subtitle` выше `head_topic`, `head_topic` выше связанного `geotag`) и включён принудительный пересчёт `subtitle_BG` после нажатия «Создать плашки» (без `onceOnly`-пропуска).
...
- Runtime path aliases cleanup: path-ключи в host-коде переведены на секционный формат (`paths.*`, `asr.whisperxPythonPath`, `asr.runnerScriptPath`, `transcribe.alignScriptPath`); legacy flat path-alias удалены из `config.schema.json` и выровнены профильные документы.
...
- Dev launcher: добавлен `scripts/dev/start-plugin-sync-watch.ps1` как рекомендуемый запуск watch-синка для сетевых/UNC путей; `.cmd`-лаунчер переведен на вызов PowerShell-скрипта.
...
- Build NuGet resiliency: в `scripts/build.ps1` добавлен retry для `dotnet restore/publish` (`word2json`) с дефолтной политикой `3x` и паузой `10s`; добавлены параметры `-DotnetRetryCount` и `-DotnetRetryDelaySeconds` для настройки в нестабильных сетях.
...
- Docs release troubleshooting: в `docs/TROUBLESHOOTING.md` добавлен отдельный блок по типовым ошибкам release workflow (confirm publish, main-branch guard, `.aex` guard, main-lineage, version guards, release-repo publish failures), плюс ссылка из `docs/RELEASE_AUTOMATION.md`.
...
- Release .NET install-dir hardening: в `release-package.yml` перед `actions/setup-dotnet@v4` добавлен шаг, который выставляет `DOTNET_INSTALL_DIR` в writable temp-каталог runner (`$env:RUNNER_TEMP\\dotnet-sdk`), чтобы publish не требовал прав записи в `C:\Program Files\dotnet` на self-hosted runner.
- Release .NET preinstalled fallback: в `release-package.yml` добавлен детект preinstalled `.NET 8 SDK` на runner; при наличии локального SDK шаг `setup-dotnet` пропускается, что снижает зависимость publish от сетевой установки .NET.
...
- Subtitles UI: устранено повторное создание тех же блоков по `Space` сразу после `Regular/Italic` — кнопки генерации теперь снимают фокус после клика и имеют защиту от повторного запуска, пока предыдущая генерация не завершена.
