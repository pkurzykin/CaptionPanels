...
static SPBasicSuite* s_pica_basicP = nullptr;
static AEGP_PluginID s_plugin_id = 0;

SPBasicSuite* CaptionPanels_GetPicaBasic()
{
	return s_pica_basicP;
}

AEGP_PluginID CaptionPanels_GetPluginId()
{
	return s_plugin_id;
}
...
public:
	...
	{
		PT_XTE_START {
			reinterpret_cast<CaptionPanels*>(refconP)->CommandHook(command, hook_priority, already_handledB, handledPB);
	...
	{
		PT_XTE_START {
			reinterpret_cast<CaptionPanels*>(plugin_refconP)->UpdateMenuHook(active_window);
		} PT_XTE_CATCH_RETURN_ERR;
	}
	...
	{
		PT_XTE_START {
			 reinterpret_cast<CaptionPanels*>(plugin_refconP)->CreatePanelHook(container, panelH, outFunctionTable, outRefcon);
		} PT_XTE_CATCH_RETURN_ERR;
	}
	...
	void CreatePanelHook(							
		...
		AEGP_PanelRefcon*		outRefcon)
	{
		*outRefcon = reinterpret_cast<AEGP_PanelRefcon>(new CaptionPanelsUI_Plat(i_pica_basicP, panelH, container, outFunctionTable));
	}
};
...
{
	PT_XTE_START{
		*global_refconP = (AEGP_GlobalRefcon) new CaptionPanels(pica_basicP, aegp_plugin_id);
	} PT_XTE_CATCH_RETURN_ERR;
}
