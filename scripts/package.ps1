...
function Acquire-ExclusiveFileLock {
    ...
    try {
        ...
        throw "Another packaging process is already running (lock: $LockPath). Wait until it completes and retry."
    }
...
$packageLockPath = Join-Path $distRoot ".package.lock"
$packageLock = $null
try {
    $packageLock = Acquire-ExclusiveFileLock -LockPath $packageLockPath
