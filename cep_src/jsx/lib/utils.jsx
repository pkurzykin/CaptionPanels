...
fixTypographyText = function (txt) {
    ...
    // Нормализуем "тире" к одному символу (–).
    // ВАЖНО: это не трогает обычный дефис "-" (он нужен для слов типа "что-то").
    // Покрываем самые частые "странные" варианты: em dash, horizontal bar, minus sign, box-drawing lines, и т.п.
    t = t.replace(/[\u2012\u2013\u2014\u2015\u2212\uFE58\u2E3A\u2E3B\u2500\u2501]/g, "–");

    // Защита бренда: "Транснефть - <слово>" не должен разрываться переносом строки
    // Работает и вне кавычек «…».
    t = t.replace(/Транснефть[ \t\u00A0]*[–—-][ \t\u00A0]*([A-Za-zА-Яа-яЁё][A-Za-zА-Яа-яЁё0-9._-]*)/g, function (_, tail) {
        return "Транснефть\u00A0–\u00A0" + tail;
    });

    // Трим по краям
    t = t.replace(/^\s+|\s+$/g, "");

    // Строка не может начинаться с пробела/табуляции (в т.ч. после переноса строки).
    t = t.replace(/(^|[\r\n])[ 	\u00A0]+/g, "$1");
